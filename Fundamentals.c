...
/* DIM_A == 4 or DIM_A == 8 */
Mat *matA, *matInvA;
Mat *matTransA, *matHat, *matGrave, *matAcute;
#if !DIVIDE
...
/* **DEPRECATED** :  old-version of bitAndWithMask */
...
//
//	Mat *matTij[MASKD_SQUARE], *matR[MASKD_SQUARE];
//	int i, j, k;
//	int index;   
//	Mat *matTem;
//	int slicesIndex;
...
/* **DEPRECATED** :  old-version of 16x16  */
#if !DIVIDE && DIM_L == 16 && DIM_A
Mat **bitAndWithMask(
	const Mat **matEX,
	const Mat **matEY
...
#if DIM_A
			if (i == 0 && j == 0){
				matTij[index] = bitAndNew_AA(matEX[i], matEY[j]);
			}
			else if (i == 0){
				matTij[index] = bitAndNew_EA(matEY[j], matEX[i]);
			}
			else if (j == 0){
				matTij[index] = bitAndNew_EA(matEX[i], matEY[j]);
			}
			else {/*  i != 0 && j != 0  */
				matTij[index] = bitAnd(matEX[i], matEY[j]);
			}
	...
	for (i = 0; i < MASKD; ++i){
		...
		for (j = i + 1; j < MASKD; ++j){
			...
			if (i == 0){
				index = j;
				matTem = matR[index];
				matR[index] = multiply(matTem, matAs);
				deMat(matTem);
			}
#endif  /* DIM_A != 0 */
		}
...
//				if (i == 0 && j == 0){
//
//					matTij[index] = bitAndNew_AA(matEXPart[i], matEYPart[j]);
//				}
//				else if (i == 0){
//
//					matTij[index] = bitAndNew_EA(matEYPart[j], matEXPart[i]);
//				}
//				else if (j == 0){
//
//					matTij[index] = bitAndNew_EA(matEXPart[i], matEYPart[j]);
//				}
//				else {/*  i != 0 && j != 0  */
//					matTij[index] = bitAnd(matEXPart[i], matEYPart[j]);
//				}
...
/* **DEPRECATED** : old-version of matrix multiplication (original) */
